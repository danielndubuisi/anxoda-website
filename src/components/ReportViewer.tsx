...
import { ArrowLeft, BarChart3, FileText, Lightbulb, Download, Calendar, Database, TrendingUp } from 'lucide-react';
...
export const ReportViewer: React.FC<ReportViewerProps> = ({ reportId, onBack }) => {
  ...
  const renderChart = (chartConfig: any, index: number) => {
    if (!chartConfig || !chartConfig.data || chartConfig.data.length === 0) {
      return <div className="text-center text-muted-foreground py-8">No data available for this chart</div>;
    }
        ...
        return (
          <ResponsiveContainer {...commonProps}>
            <BarChart data={chartConfig.data}>
              ...
              />
              <Bar 
                dataKey={chartConfig.yKey || chartConfig.data[0]?.count !== undefined ? "count" : 
  ...
  const parseAnalysisData = (summary: Json): AnalysisData => {
    ...
    return {
      keyFindings: [typeof summary === 'string' ? summary : 'No analysis available'],
      additionalKPIs: [],
      recommendations: []
    };
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{report.row_count?.toLocaleString() || 'N/A'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{report.column_count || 'N/A'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="visualization" className="space-y-6">
        ...
        <TabsContent value="visualization" className="space-y-6">
          {report.chart_data && Array.isArray(report.chart_data) && report.chart_data.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {(report.chart_data as any[]).map((chart, index) => (
                <Card key={index}>
                  <CardHeader>
                    <CardTitle className="text-lg">{chart.title || `Chart ${index + 1}`}</CardTitle>
                    {chart.description && (
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-16">
                ...
                <p className="text-muted-foreground text-center">
                  Charts and graphs will appear here once the data analysis is complete.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {analysisData.keyFindings.length > 0 ? (
                ...
                <p className="text-muted-foreground">No analysis summary available.</p>
              )}
            </CardContent>
          </Card>
          ...
          {analysisData.additionalKPIs.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Other relevant metrics discovered in your data
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {analysisData.additionalKPIs.map((kpi, index) => (
        ...
        <TabsContent value="recommendations" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Data-driven suggestions to improve your business performance
              </CardDescription>
            </CardHeader>
