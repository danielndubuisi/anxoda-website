...
import { FileSpreadsheet, Calendar, BarChart, Trash2, Eye, Download } from 'lucide-react';
...
export const ReportList: React.FC<ReportListProps> = ({ onViewReport, refreshTrigger }) => {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await fetch(`https://pjevyfyvrgvjgspxfikd.supabase.co/functions/v1/get-reports`, {
        headers: {
  ...
  const deleteReport = async (reportId: string) => {
    ...
    try {
      ...
      const response = await fetch(`https://pjevyfyvrgvjgspxfikd.supabase.co/functions/v1/delete-report`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
      ...
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-muted-foreground">
            Upload your first spreadsheet to get started with AI-powered analysis.
          </p>
        </CardContent>
      </Card>
    );
